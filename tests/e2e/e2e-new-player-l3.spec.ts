...
test.describe('@phase14-smoke', () => {
  test('new player can complete the short MVP L3 loop', async ({ page }) => {
    ...
    // Smoke: verify canvas loaded and player id stored
    const storedId = await page.evaluate((key) => localStorage.getItem(key), PLAYER_ID_KEY);
    expect(storedId).toBe(PLAYER_ID);

    // Smoke: trigger WS message flow via game events — client must handle server quest/shop/level messages
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent('game:npc-interact', { detail: { npc_id: 'hopper' } }));
    });

    // Short verification: canvas stays visible with no page errors throughout
    ...
    expect(
      pageErrors,
      `Smoke test console errors: ${pageErrors.join('; ')}`,
    ).toHaveLength(0);
  });
});
