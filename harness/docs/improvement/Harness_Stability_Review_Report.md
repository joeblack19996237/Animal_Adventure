...
Suggested Fix Order
Harden stop_git_commit.py path validation and git add -- usage.
Add runtime phase/task signal validation in both phase_handlers.py and the stop commit hook.
Improve task-level blocked status reporting while preserving the intended retry behavior.
Rework subprocess timeout cleanup around process groups.
Add PID creation-time/owner-token checks to run lock handling.
Tests To Add
